    ...
    <Box className="min-h-screen bg-[#0D0D0C] text-gray-200">
      ...
      <Container maxWidth="md" className="py-8 px-4 space-y-6">
        ...
        <Box className="bg-[#141413] border border-[#D4AF37]/10 p-6 rounded-xl space-y-6">
          ...
          <Box className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-black/30 p-3.5 rounded border border-[#D4AF37]/5">
            ...
            <span className="text-[10px] font-mono text-gray-500">
              Last Sync: {formatDateToDDMMYYYY(booking.updatedAt || booking.createdAt)} {new Date(booking.updatedAt || booking.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </span>
          </Box>
        ...
        <Box className="bg-[#141413] border border-[#D4AF37]/10 p-6 rounded-xl space-y-4">
          ...
          {(() => {
            const isDualAlbum = booking.packageName?.toLowerCase().includes('both side') || false;
            const albumIndices: (1 | 2)[] = isDualAlbum ? [1, 2] : [1];

            // If none of the albums are uploaded
            const hasAnyAlbum = (booking.albumDesignPdfUrl || (isDualAlbum && booking.albumDesignPdfUrl2));
            ...
            return (
              <div className="space-y-4">
                {albumIndices.map((albumIdx) => {
                  const pdfUrl = albumIdx === 1 ? booking.albumDesignPdfUrl : booking.albumDesignPdfUrl2;
                  const status = albumIdx === 1 ? booking.albumDesignStatus : booking.albumDesignStatus2;
                  const uploadDate = albumIdx === 1 ? booking.albumDesignUploadDate : booking.albumDesignUploadDate2;
                  const notes = albumIdx === 1 ? booking.albumDesignNotes : booking.albumDesignNotes2;

                  if (!pdfUrl) {
                    return (
                      <div key={albumIdx} className="p-4 bg-black/10 border border-dashed border-[#D4AF37]/10 rounded-lg space-y-2 text-left">
                        <Typography variant="subtitle2" className="text-white font-serif font-bold text-xs uppercase tracking-wider flex items-center gap-2">
                          <FileText className="w-3.5 h-3.5 text-[#D4AF37]" />
                          <span>{isDualAlbum ? `Album Design ${albumIdx}` : 'Album Design'}</span>
                        </Typography>
                        <Typography variant="body2" className="text-gray-500 text-xs">
                          This album design is not available yet. Our design team is working on it!
                        </Typography>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div key={albumIdx} className="p-4 bg-black/15 border border-[#D4AF37]/10 rounded-lg space-y-4 text-left">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-black/20 p-4 rounded-lg border border-[#D4AF37]/5">
                        <div className="space-y-1">
                          ...
                          <Typography variant="body2" className="text-white font-mono text-xs">
                            {uploadDate ? formatDateToDDMMYYYY(uploadDate) : 'Not specified'}
                          </Typography>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setPreviewPdfTitle(`Album Design ${albumIdx}: ${booking.clientName}`);
                            setPreviewPdfFilename(`Album_Design_${albumIdx}_${booking.clientName.replace(/\s+/g, '_')}.pdf`);
                            setViewOpen(true);
                          }}
                        ...
                        >
                          📖 View Album {isDualAlbum ? albumIdx : ''}
                        </Button>
                        ...
                        <Button
                          ...
                          sx={{ textTransform: 'none' }}
                        >
                          📥 Download Album {isDualAlbum ? albumIdx : ''}
                        </Button>
                      </div>
                    </div>
                  );
            ...
            <div className="space-y-3.5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <Typography variant="body2" className="text-white font-serif font-semibold text-sm mt-1">{formatDateToDDMMYYYY(booking.weddingDate)}</Typography>
                </div>
              </div>
              {booking.eventTime && (
                <div className="flex items-start gap-3">
