...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const [linkModeBookingId, setLinkModeBookingId] = useState<string | null>(null);
  const [linkInputValues, setLinkInputValues] = useState<{[key: string]: string}>({});

  const handleUploadPdf = async (booking: Booking, event: React.ChangeEvent<HTMLInputElement>, albumIndex: 1 | 2 = 1) => {
    const file = event.target.files?.[0];
    setUploadError(null);

    // 4. Log step: File selected
    if (file) {
      console.log('[Upload Stage] File selected:', {
        name: file.name,
        size: file.size,
        type: file.type,
      ...
      console.warn('[Upload Stage] No file selected.');
      return;
    }

    // 8. Check file size and MIME type validation
    if (file.type !== 'application/pdf') {
      const errMsg = `Validation failed: Selected file has type "${file.type}", but only PDF files are supported.`;
    ...
    if (file.size > MAX_FILE_SIZE) {
      const errMsg = `Validation failed: File size is ${file.size} bytes (${(file.size / (1024 * 1024)).toFixed(2)}MB), which exceeds the 50MB limit.`;
      console.error('[Upload Fail]', errMsg);
      setUploadError(errMsg);
    ...
    const uploadId = `${booking.id}_album_${albumIndex}`;
    setUploadingBookingId(uploadId);
    setUploadProgress(0);

    try {
      // 1. Verify Firebase Storage is initialized correctly
      if (!storage) {
        throw new Error('Firebase Storage SDK has not been initialized correctly. The storage object is undefined.');
      }

      // 2. Verify the correct Storage Bucket is configured
      const bucketName = storage.app?.options?.storageBucket;
      if (!bucketName) {
        throw new Error('Firebase configuration is missing the "storageBucket" property.');
      }
      console.log('[Upload Stage] Firebase Storage Bucket Verified:', bucketName);

      // 7. Check Authentication permission
      const currentUser = auth.currentUser;
      if (!currentUser) {
        console.warn('[Upload Stage] User is NOT authenticated in Firebase Auth. Upload will likely fail due to security rules.');
        throw new Error('Authentication required: You must be logged in to upload files. Firebase Storage security rules require authenticated access.');
      } else {
        console.log('[Upload Stage] User is authenticated:', {
          uid: currentUser.uid,
      ...
      const filePath = `album_designs/${booking.id}_album_${albumIndex}_${Date.now()}.pdf`;
      const metadata = { contentType: 'application/pdf' };
      let downloadUrl = '';

      const performUpload = (storageInstance: any, isFallback: boolean): Promise<string> => {
        return new Promise<string>((resolve, reject) => {
          const fileRef = ref(storageInstance, filePath);
          console.log(`[Upload Stage] Initiating upload task (${isFallback ? 'Fallback' : 'Primary'})...`);
          const task = uploadBytesResumable(fileRef, file, metadata);
          
          // Standard timeout (60 seconds) for the upload attempt to handle realistic file sizes/networks
          const timeoutMs = 60000;
          const timeoutId = setTimeout(() => {
            console.error(`[Upload Fail] ${isFallback ? 'Fallback' : 'Primary'} upload timed out after ${timeoutMs / 1000}s.`);
            task.cancel();
          ...
          task.on(
            ...
            (snapshot) => {
              ...
              console.log(`[Upload Stage] Progress update: ${progressRound}% (${bytesTransferred}/${totalBytes} bytes), state: ${snapshot.state}`);
              setUploadProgress(progressRound);
            },
            (error) => {
              clearTimeout(timeoutId);
              console.error(`[Upload Fail] ${isFallback ? 'Fallback' : 'Primary'} returned error:`, error);
              reject(error);
            },
            async () => {
              clearTimeout(timeoutId);
              console.log(`[Upload Stage] ${isFallback ? 'Fallback' : 'Primary'} completed successfully.`);
              try {
      ...
      try {
        downloadUrl = await performUpload(storage, false);
      } catch (err: any) {
        if (err.message === 'TIMEOUT' || err.code === 'storage/unknown' || err.message?.includes('network') || err.message?.includes('unauthorized')) {
          console.log('[Upload Stage] Primary upload failed, timed out, or unauthorized. Trying appspot.com fallback storage...');
          try {
            const appspotStorage = getStorage(storage.app, `gs://${storage.app.options.projectId}.appspot.com`);
            downloadUrl = await performUpload(appspotStorage, true);
          } catch (fallbackErr: any) {
            console.error('[Upload Stage] Fallback upload also failed:', fallbackErr);
            throw new Error(`Upload failed. We tried both storage buckets (${storage.app.options.storageBucket} and ${storage.app.options.projectId}.appspot.com). Please make sure Firebase Storage is enabled in your Firebase console, or use the "Paste Link" option instead.`);
          }
        } else {
          throw err;
        }
      ...
      // 9. Ensure the Download URL is saved to the correct booking document
      console.log('[Upload Stage] Preparing booking object update...');
      ...
      const updatedBooking: Booking = {
        ...
        ...(albumIndex === 1 ? {
          ...
          albumDesignUploadDate: dateString
        } : {
          albumDesignPdfUrl2: downloadUrl,
          albumDesignStatus2: booking.albumDesignStatus2 || 'Waiting for Client Review',
          albumDesignUploadDate2: dateString
        }),
        updatedAt: Date.now()
      };

      console.log('[Upload Stage] Saving updated booking to local and cloud stores...');
      await offlineService.updateBooking(updatedBooking);
      console.log('[Upload Stage] Booking document updated successfully in DB.');

      setBookings(prev => prev.map(b => b.id === booking.id ? updatedBooking : b));
      setSnackbarMessage(`Album design ${albumIndex} PDF uploaded successfully!`);
      setSnackbarOpen(true);
      
      // Clear uploading/progress/error states since it succeeded
      setUploadingBookingId(null);
      setUploadProgress(null);
      setUploadError(null);
    } catch (error: any) {
      console.error('[Upload Error] Exception occurred during upload process:', error);
      const errMsg = error.message || String(error);
      setUploadError(errMsg);
      setSnackbarMessage(`Failed to upload PDF: ${errMsg}`);
      setSnackbarOpen(true);
      setUploadProgress(null);
    }
  };

  const handleSavePdfUrl = async (booking: Booking, url: string, albumIndex: 1 | 2 = 1) => {
    if (!url || !url.trim()) {
      setSnackbarMessage('Please enter a valid PDF URL.');
      setSnackbarOpen(true);
      return;
    }
    ...
    const updatedBooking: Booking = {
      ...booking,
      ...(albumIndex === 1 ? {
        albumDesignPdfUrl: url.trim(),
        albumDesignStatus: booking.albumDesignStatus || 'Waiting for Client Review',
        albumDesignUploadDate: dateString
      } : {
        albumDesignPdfUrl2: url.trim(),
        albumDesignStatus2: booking.albumDesignStatus2 || 'Waiting for Client Review',
        albumDesignUploadDate2: dateString
      }),
      updatedAt: Date.now()
    ...
    try {
      await offlineService.updateBooking(updatedBooking);
      setBookings(prev => prev.map(b => b.id === booking.id ? updatedBooking : b));
      setSnackbarMessage(`Album design ${albumIndex} PDF link saved successfully!`);
      setSnackbarOpen(true);
    } catch (error: any) {
      console.error('[Save URL Error]', error);
      setSnackbarMessage(`Failed to save URL: ${error.message || String(error)}`);
      setSnackbarOpen(true);
    }
  };

  const handleDeletePdf = async (booking: Booking, albumIndex: 1 | 2 = 1) => {
    if (!window.confirm(`Are you sure you want to delete the Album Design ${albumIndex} PDF for this booking?`)) {
      return;
    }
    ...
    setUploadingBookingId(uploadId);
    try {
      const targetUrl = albumIndex === 1 ? booking.albumDesignPdfUrl : booking.albumDesignPdfUrl2;
      if (targetUrl) {
        try {
      ...
      const updatedBooking: Booking = {
        ...
        ...(albumIndex === 1 ? {
          ...
          albumDesignPdfUrl2: undefined,
          albumDesignStatus2: 'Not Uploaded',
          albumDesignUploadDate2: undefined
        }),
        updatedAt: Date.now()
      };
      ...
      await offlineService.updateBooking(updatedBooking);
      setBookings(prev => prev.map(b => b.id === booking.id ? updatedBooking : b));
      setSnackbarMessage(`Album design ${albumIndex} PDF deleted successfully.`);
      setSnackbarOpen(true);
    } catch (error: any) {
                          ...
                          <Box>
                            ...
                            <Box className="flex items-center gap-3 mt-0.5 text-[11px] text-gray-500 font-medium">
                              <span className="flex items-center gap-1">
                                ...
                                {formatDateToDDMMYYYY(b.weddingDate)}
                              </span>
                              <span>•</span>
                ...
                return (
                  <Box key={b.id} className="p-4 rounded-xl bg-black/35 border border-[#D4AF37]/10 space-y-3">
                    ...
                    <Box className="mt-4 pt-4 border-t border-[#D4AF37]/10 space-y-4">
                      {(() => {
                        const isDualAlbum = b.packageName?.toLowerCase().includes('both side') || false;
                        const albumIndices: (1 | 2)[] = isDualAlbum ? [1, 2] : [1];

                        return albumIndices.map((albumIdx) => {
                          const pdfUrl = albumIdx === 1 ? b.albumDesignPdfUrl : b.albumDesignPdfUrl2;
                          const status = albumIdx === 1 ? b.albumDesignStatus : b.albumDesignStatus2;
                          const uploadDate = albumIdx === 1 ? b.albumDesignUploadDate : b.albumDesignUploadDate2;
                          const uploadId = `${b.id}_album_${albumIdx}`;

                          return (
                            <div key={albumIdx} className="space-y-3 bg-black/10 p-3.5 rounded-lg border border-[#D4AF37]/10">
                              <div className="flex items-center gap-2 border-b border-[#D4AF37]/5 pb-2">
                                <FileText className="w-4 h-4 text-[#D4AF37]" />
                                <Typography variant="subtitle2" className="text-white font-serif font-bold text-xs uppercase tracking-wider">
                                  📄 {isDualAlbum ? `Album Design ${albumIdx}` : 'Album Design'}
                                </Typography>
                              </div>
                              ...
                              {pdfUrl ? (
                                <div className="space-y-3 bg-black/25 p-3 rounded-lg border border-[#D4AF37]/5">
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <div className="space-y-1 flex-1">
                                      <div className="flex items-center gap-1.5 text-[10px] text-gray-400 font-mono">
                                        ...
                                        <span>Uploaded: {uploadDate ? formatDateToDDMMYYYY(uploadDate) : 'Unknown'}</span>
                                      </div>
                                      <div className="flex items-center gap-2 flex-wrap">
                                        <span className="text-[10px] text-gray-400">Status:</span>
                                        <Chip
                                          label={status === 'Approved' ? 'Album Approved' : (status || 'Waiting for Client Review')}
                                          size="small"
                                          className={`text-[9px] h-4.5 font-bold uppercase tracking-wider ${
                                            status === 'Album Approved' || status === 'Approved' ? 'bg-green-500/15 text-green-400 border border-green-500/20' :
                                            status === 'Changes Requested' ? 'bg-red-500/15 text-red-400 border border-red-500/20' :
                                            status === 'Client Reviewing' ? 'bg-blue-500/15 text-blue-400 border border-blue-500/20' :
                                            status === 'Not Uploaded' ? 'bg-stone-500/15 text-stone-400 border border-stone-500/20' :
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <Select
                                        value={status === 'Approved' ? 'Album Approved' : (status || 'Waiting for Client Review')}
                                        onChange={async (e) => {
                                          const updated = {
                                            ...b,
                                            ...(albumIdx === 1 ? {
                                              ...
                                              albumDesignStatus2: e.target.value as any
                                            })
                                          };
                                          await offlineService.updateBooking(updated);
                                          setSnackbarMessage(`Album design ${albumIdx} status updated successfully.`);
                                          setSnackbarOpen(true);
                                        }}
                                  ...
                                  <div className="flex flex-wrap gap-2 pt-1 border-t border-gray-900">
                                    <Button
                                      ...
                                      onClick={() => {
                                        ...
                                        setPreviewPdfTitle(`Album Design ${albumIdx}: ${b.clientName}`);
                                        setPreviewPdfFilename(`Album_Design_${albumIdx}_${b.clientName.replace(/\s+/g, '_')}.pdf`);
                                        setPreviewOpen(true);
                                      }}
                                    ...
                                    <label className="cursor-pointer">
                                      <input
                                        ...
                                        className="hidden"
                                        onChange={(e) => handleUploadPdf(b, e, albumIdx)}
                                      />
                                      <Button
                                        size="small"
                                        variant="outlined"
                                        component="span"
                                        disabled={uploadingBookingId === uploadId}
                                        startIcon={uploadingBookingId === uploadId ? <CircularProgress size={12} color="inherit" /> : <RefreshCw className="w-3.5 h-3.5" />}
                                        className="text-xs text-amber-400 border-amber-950/40 hover:bg-amber-500/10 h-7"
                                        sx={{ textTransform: 'none' }}
                                      >
                                        {uploadingBookingId === uploadId ? `Replacing (${uploadProgress !== null ? uploadProgress : 0}%)` : 'Replace PDF'}
                                      </Button>
                                    </label>
                                    ...
                                    <Button
                                      ...
                                      variant="outlined"
                                      color="error"
                                      onClick={() => handleDeletePdf(b, albumIdx)}
                                      disabled={uploadingBookingId === uploadId}
                                  ...
                                  {uploadingBookingId === uploadId && (
                                    <div className="space-y-1 pt-2 border-t border-gray-900/40">
                                      {uploadError ? (
                                        <div className="space-y-1.5 text-left bg-red-950/15 p-2 rounded border border-red-500/20">
                                          <span className="text-[10px] text-red-400 font-mono block font-semibold leading-relaxed">Error: {uploadError}</span>
                                          <Button 
                                            size="small" 
                                            variant="text" 
                                            className="text-[10px] text-gray-400 p-0 min-w-0 normal-case hover:text-white"
                                            onClick={() => {
                                              setUploadingBookingId(null);
                                              setUploadProgress(null);
                                              setUploadError(null);
                                        ...
                                        uploadProgress !== null && (
                                          <>
                                            <div className="w-full bg-[#12110D] rounded-full h-1.5 overflow-hidden border border-[#D4AF37]/10">
                                              <div className="bg-gradient-to-r from-[#AA7C11] via-[#D4AF37] to-[#FFF] h-full transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                                            </div>
                                            <span className="text-[10px] text-gray-400 font-mono block text-left">Replacing: {uploadProgress}%</span>
                                          </>
                                        )
                                      )}
                                    </div>
                                ...
                                linkModeBookingId === uploadId ? (
                                  <div className="space-y-3 bg-black/40 p-4 rounded-lg border border-[#D4AF37]/20">
                                    <Typography variant="body2" className="text-[#D4AF37] text-xs font-semibold text-left">
                                      Paste PDF URL
                                    </Typography>
                                    <div className="flex gap-2">
                                      <input
                                        type="text"
                                        placeholder="https://example.com/album.pdf"
                                        value={linkInputValues[uploadId] || ''}
                                        onChange={(e) => {
                                          const val = e.target.value;
                                          setLinkInputValues(prev => ({ ...prev, [uploadId]: val }));
                                        }}
                                        className="flex-1 bg-[#12110D] text-gray-200 border border-[#D4AF37]/20 rounded px-2.5 py-1.5 text-xs focus:outline-none focus:border-[#D4AF37]/60"
                                      />
                                      <Button
                                        size="small"
                                        variant="contained"
                                        onClick={() => {
                                          handleSavePdfUrl(b, linkInputValues[uploadId] || '', albumIdx);
                                          setLinkModeBookingId(null);
                                        }}
                                        className="bg-[#D4AF37] hover:bg-[#AA7C11] text-black text-xs font-bold font-sans h-8 px-3"
                                      ...
                                      <Button
                                        size="small"
                                        variant="outlined"
                                        onClick={() => setLinkModeBookingId(null)}
                                        className="border-gray-800 text-gray-400 hover:bg-gray-900 text-xs h-8 px-3"
                                        sx={{ minWidth: 'auto', textTransform: 'none' }}
                                      >
                                        Cancel
                                      </Button>
                                    </div>
                                    <Typography variant="caption" className="text-gray-500 text-[10px] block text-left">
                                      Tip: Useful if Firebase Storage bucket is not yet provisioned in your Firebase Console.
                                    </Typography>
                                  </div>
                                ) : (
                                  <div className="bg-black/25 border border-dashed border-[#D4AF37]/15 rounded-lg p-4 text-center space-y-3">
                                    <Typography variant="body2" className="text-gray-500 text-xs">
                                      No {isDualAlbum ? `Album Design ${albumIdx}` : 'Album Design'} PDF has been uploaded for this booking.
                                    </Typography>
                                    {uploadingBookingId === uploadId && (
                                      <div className="space-y-1 max-w-xs mx-auto">
                                        {uploadError ? (
                                          <div className="space-y-1.5 text-center bg-red-950/15 p-2 rounded border border-red-500/20">
                                            <span className="text-[10px] text-red-400 font-mono block font-semibold leading-relaxed">Error: {uploadError}</span>
                                            <Button 
                                              size="small" 
                                              variant="text" 
                                          ...
                                          uploadProgress !== null && (
                                            <>
                                              ...
                                              <span className="text-[10px] text-gray-400 font-mono">Uploading: {uploadProgress}%</span>
                                            </>
                                          )
                                        )}
                                      </div>
                                    ...
                                    <div className="flex justify-center gap-2.5">
                                      <label className="cursor-pointer">
                                        ...
                                        <Button
                                          ...
                                          component="span"
                                          disabled={uploadingBookingId === uploadId}
                                          startIcon={uploadingBookingId === uploadId ? <CircularProgress size={12} color="inherit" /> : <Upload className="w-3.5 h-3.5" />}
                                          className="border-[#D4AF37]/30 text-[#D4AF37] hover:bg-[#D4AF37]/10 text-xs px-4 h-8"
                                          sx={{ textTransform: 'none' }}
                                        >
                                          {uploadingBookingId === uploadId ? `Uploading (${uploadProgress !== null ? uploadProgress : 0}%)` : `Upload PDF`}
                                        </Button>
                                      </label>
                                      <Button
                                        size="small"
                                        ...
                                        disabled={uploadingBookingId === uploadId}
                                        onClick={() => {
                                          setLinkModeBookingId(uploadId);
                                          setLinkInputValues(prev => ({ ...prev, [uploadId]: '' }));
                                        }}
                                        startIcon={<ExternalLink className="w-3.5 h-3.5" />}
                                        className="border-amber-950/40 text-amber-500 hover:bg-amber-500/10 text-xs px-4 h-8"
                                        sx={{ textTransform: 'none' }}
                                      >
                                        Paste Link
                                      </Button>
