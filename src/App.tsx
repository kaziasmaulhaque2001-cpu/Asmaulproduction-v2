...
const AppContent: React.FC = () => {
  ...
  const sidebarItems = [
    ...
    { value: 'ai-consultant', label: 'AI Studio Advisor', icon: <Sparkles className="w-5 h-5 text-[#D4AF37] animate-pulse" /> },
        ...
        {detailedBooking && (
          <>
            ...
            <DialogContent className="pt-4 space-y-4">
              ...
              <List className="p-0 space-y-2">
                <ListItem className="px-0 py-1.5 border-b border-gold-glow/5">
                  ...
                  <ListItemText 
                    ...
                    secondary={<span className="text-white font-serif font-semibold text-sm block mt-0.5">{formatDateToDDMMYYYY(detailedBooking.weddingDate)}</span>} 
                  />
                </ListItem>
                <ListItem className="px-0 py-1.5 border-b border-gold-glow/5">
              ...
              {/* AI Creative Assist Panel */}
              <Box className="mt-4 p-3 bg-[#12110D] border border-[#D4AF37]/25 rounded-xl space-y-2.5">
                <Box className="flex items-center justify-between">
                  <Box className="flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-[#D4AF37] animate-pulse" />
                    <Typography variant="caption" className="text-[#D4AF37] font-bold font-serif uppercase tracking-wider block">
                      Kazi Creative Assist
                    </Typography>
                  </Box>
                  {aiGenerating && <CircularProgress size={12} className="text-[#D4AF37]" />}
                </Box>
                ...
                <Box className="flex flex-wrap gap-2">
                  <Button 
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const promptText = `Generate a cinematic, hour-by-hour timeline schedule for a photography package named "${detailedBooking.packageName}" for client "${detailedBooking.clientName}" at location "${detailedBooking.venue}". Structure it with beautiful timing guidelines. Keep it to about 150 words.`;
                        const res = await fetch('/api/gemini/chat', {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({
                            contents: [{ role: 'user', parts: [{ text: promptText }] }],
                            systemInstruction: 'You are Kazi, the lead creative scheduler for Asmaul Production. Respond with a direct, beautiful schedule.',
                          })
                        });
                    ...
                    className="border-[#D4AF37]/20 hover:border-[#D4AF37]/50 hover:bg-[#D4AF37]/5 text-[#D4AF37] py-1 px-2.5 rounded-lg font-sans text-[10px] uppercase font-semibold"
                  >
                    Generate Timeline
                  </Button>
                  ...
                  <Button 
                    ...
                    onClick={async () => {
                      setAiGenerating(true);
                      try {
                        const promptText = `Suggest a customized premium shot list of key poses, lighting, and frame composition ideas for the photography package "${detailedBooking.packageName}" for client "${detailedBooking.clientName}". Keep it to 150 words.`;
                        const res = await fetch('/api/gemini/chat', {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({
                            contents: [{ role: 'user', parts: [{ text: promptText }] }],
                            systemInstruction: 'You are Kazi, the luxury photography director for Asmaul Production. Respond with key creative shot suggestions.',
                          })
                        });
                        const data = await res.json();
                        if (data.text) {
                ...
                {aiTextResult && (
                  <Box className="space-y-2 mt-2">
                    <Box className="p-3 bg-black/45 rounded border border-[#D4AF37]/15 text-xs text-gray-200 leading-relaxed max-h-48 overflow-y-auto whitespace-pre-line text-left">
                      {aiTextResult}
                    </Box>
                    <Box className="flex justify-end gap-2">
                      <Button
                        size="small"
                        onClick={() => setAiTextResult('')}
                      ...
                      <Button
                        ...
                        size="small"
                        onClick={async () => {
                          const originalNotes = detailedBooking.notes ? detailedBooking.notes + "\n\n" : "";
                          const updatedBooking = { ...detailedBooking, notes: originalNotes + "=== AI SUGGESTED DIRECTIVE ===\n" + aiTextResult };
                          setDetailedBooking(updatedBooking);
                          await offlineService.updateBooking(updatedBooking);
                          setAiTextResult('');
                          triggerRefresh();
                        }}
                        className="bg-[#D4AF37] hover:bg-[#AA7C11] text-black text-[10px] font-bold rounded-lg py-1 px-2.5"
                      >
                        Append to Directives
                      </Button>
                    </Box>
                  </Box>
                )}
